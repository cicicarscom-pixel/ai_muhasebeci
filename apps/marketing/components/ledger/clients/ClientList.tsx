...
export function ClientList({ clients, selectedClientId, onSelect, query, onQueryChange }: ClientListProps) {
  return (
    <div className="flex flex-col rounded-[16px] border border-white/5 bg-[#12151C] overflow-hidden shadow-[0_4px_20px_rgba(0,0,0,0.2)]">
      {/* Search Bar */}
      <div className="border-b border-white/5 bg-[#161B22] p-4 sticky top-0 z-10">
        <div className="relative">
          ...
          <input
            ...
            className="h-10 w-full rounded-xl border border-white/10 bg-[#12151C] pl-9 pr-4 text-sm text-white placeholder:text-[#8B949E] focus:border-cyan-400/30 focus:outline-none transition-colors"
          />
        </div>
      </div>

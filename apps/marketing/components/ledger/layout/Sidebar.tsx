...
function SidebarItem({ icon, label, href, active }: { icon: string, label: string, href: string, active?: boolean }) {
  return (
    <Link href={href} className={`relative flex items-center justify-center w-[48px] h-[48px] rounded-[14px] group/item transition-all duration-medium ease-in-out ${active ? 'bg-primary/10 border-l-[3px] border-l-primary' : 'hover:bg-card/50 hover:scale-[1.02]'}`}>
